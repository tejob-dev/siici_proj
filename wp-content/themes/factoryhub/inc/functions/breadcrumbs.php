 ...
 */
function factoryhub_breadcrumbs( $args = '' ) {
	$args = wp_parse_args( $args, array(
		...
		'show_on_front'     => false,
		'labels'            => array(
			'home'      => esc_html__( 'Home', 'factoryhub' ),
			'archive'   => esc_html__( 'Archives', 'factoryhub' ),
			'blog'      => esc_html__( 'Blog', 'factoryhub' ),
			'search'    => esc_html__( 'Search results for', 'factoryhub' ),
			'not_found' => esc_html__( 'Not Found', 'factoryhub' ),
			'author'    => esc_html__( 'Author Archives:', 'factoryhub' ),
			'day'       => esc_html__( 'Daily Archives:', 'factoryhub' ),
			'month'     => esc_html__( 'Monthly Archives:', 'factoryhub' ),
			'year'      => esc_html__( 'Yearly Archives:', 'factoryhub' ),
		),
	) );

	$args = apply_filters( 'factoryhub_breadcrumbs_args', $args );
	...
	elseif ( is_single() ) {
		...
		if( $terms ) {
			$term    = current( $terms );
			$terms   = factoryhub_get_term_parents( $term->term_id, $taxonomy );
			$terms[] = $term->term_id;
	...
	// Page
	elseif ( is_page() ) {
		$pages = factoryhub_get_post_parents( get_queried_object_id() );
	...
	elseif ( is_tax() || is_category() || is_tag() ) {
		...
		if( $current_term ) {
			$terms        = factoryhub_get_term_parents( get_queried_object_id(), $current_term->taxonomy );
			if( $terms ) {
				foreach ( $terms as $term_id )
				{
	...
	elseif ( is_day() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'factoryhub' ), $args['labels']['day'], get_the_date() )
		);
	}
	// Month archive
	elseif ( is_month() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'factoryhub' ), $args['labels']['month'], get_the_date( 'F Y' ) )
		);
	}
	// Year archive
	elseif ( is_year() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'factoryhub' ), $args['labels']['year'], get_the_date( 'Y' ) )
		);
	}
	// Archive
	elseif ( is_post_type_archive( 'project' ) ) {
		...
		if ( $portfolio_page_id && get_post( $portfolio_page_id ) ) {
			...
			$title = _x( 'Project', 'Project post type breadcrumb', 'factoryhub' );
		}
		...
		if ( $service_page_id && get_post( $service_page_id ) ) {
			...
			$title = _x( 'Service', 'Service post type breadcrumb', 'factoryhub' );
		}
		...
		if ( $testimonial_page_id && get_post( $testimonial_page_id ) ) {
			...
			$title = _x( 'Testimonial', 'Testimonial post type breadcrumb', 'factoryhub' );
		}
 ...
 */
function factoryhub_get_term_parents( $term_id = '', $taxonomy = 'category' ) {
 ...
 */
function factoryhub_get_post_parents( $post_id = '' ) {
